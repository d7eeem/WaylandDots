...
# Build URL for cache proxy
if [[ -n "$LAT" ]] && [[ -n "$LON" ]]; then
  # Custom location provided via arguments - bypass cache
  URL="${CACHE_PROXY_URL}/v1/forecast?latitude=${LAT}&longitude=${LON}&current_weather=true&hourly=temperature_2m,apparent_temperature,weathercode,relativehumidity_2m,windspeed_10m,precipitation_probability,precipitation&daily=temperature_2m_max,temperature_2m_min,weathercode,precipitation_sum&timezone=auto"
  if [[ -n "$DEBUG" ]]; then
    echo "Using custom location: LAT=${LAT}, LON=${LON}" >&2
  fi
else
  # Use proxy's default location (cached) - no lat/lon in URL
  URL="${CACHE_PROXY_URL}/v1/forecast?current_weather=true&hourly=temperature_2m,apparent_temperature,weathercode,relativehumidity_2m,windspeed_10m,precipitation_probability,precipitation&daily=temperature_2m_max,temperature_2m_min,weathercode,precipitation_sum&timezone=auto"
  if [[ -n "$DEBUG" ]]; then
    echo "Using proxy's default cached location" >&2
  fi
fi
...
# ---------------- FETCH DATA
# Try to fetch from cache proxy first
...
if [[ $curl_exit -eq 0 ]] && [[ -n "$data" ]] && [[ "$data" != *"error"* ]]; then
  ...
  if [[ -n "$DEBUG" ]]; then
    echo "✗ Cache proxy fetch failed (exit: $curl_exit)" >&2
    echo "  URL: $URL" >&2
...
if [[ -n "$CITY" ]]; then
  ...
  # First, try to get custom city name from API response (set by proxy)
  api_city=$(echo "$data" | jq -r '._city // empty')
  if [[ -n "$api_city" ]]; then
    LOCATION_NAME="$api_city"
  else
    # Fall back to extracting city from timezone if available (e.g., "Asia/Riyadh" -> "Riyadh")
    api_timezone=$(echo "$data" | jq -r '.timezone // empty')
    if [[ -n "$api_timezone" ]] && [[ "$api_timezone" == *"/"* ]]; then
      LOCATION_NAME="${api_timezone##*/}"
...
if [[ -z "$current_temp" ]] || [[ -z "$current_code" ]]; then
  fail "Failed to parse weather data"
fi
